...
void DatasetDock::onActionTransRadio(){//Radio to Hrrp
    ...
    this->datasetInfo->modifyAttri("HRRP", "HRRP_"+clickedName,"PATH", destDataSetPath);
    this->reloadTreeView();
    this->datasetInfo->writeToXML(datasetInfo->defaultXmlPath);
}
...
//    Py_Finalize();
    ...
    this->datasetInfo->modifyAttri("FEATURE", "FEATURE_"+clickedName,"PATH", destDataSetPath);
    this->reloadTreeView();
}
