...
void ModelDock::importModel(string type){
    ...
    QString fileType = modelPath.split('.').last();
    if(modelPath == ""){
        QMessageBox::warning(NULL, "提示", "文件打开失败!");
        return;
    }else if(type=="FEA_OPTI"){
        if(fileType!="pth"){
            QMessageBox::warning(NULL, "提示", "特征优化模型的模型文件类型应为pth!");
            return;
        }
    }else if(fileType!="trt" && fileType!="pth" && fileType!="hdf5"){
    ...
    if(existXml){
        ...
        if(attriContents.find("type")==attriContents.end()){        //xml没有type信息
            terminal->print("添加模型成功:"+xmlPath);
            QMessageBox::information(NULL, "添加模型", "添加模型成功,但该模型说明文件没有模型type信息,请确保该模型为合适的"+QString::fromStdString(type)+"类型");
        }
