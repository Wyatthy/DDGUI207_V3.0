...
class MsmcNet_RML2016(nn.Module):
    ...
    def forward(self, x, cams=None,S_masks=None,B_masks=None):  #x最开始[110,1,128,2]
        ...
        if cams is not None:
            ...
            Sa_x_a = x*S_masks_a
            Ba_x_a = x*B_masks_a
            x = torch.concat((x,Sa_x_m,Ba_x_m), dim=1)  #注意需要将requires_grad=True #增减 ,Sa_x_a, Ba_x_a after1
            # x = torch.concat((x,Sa_x_m,Ba_x_m,Sa_x_a, Ba_x_a), dim=1)  #注意需要将requires_grad=True after2
            x  = self.flatten(x)
            x = self.fc0(x)
            x = self.relu(x)
            x = self.dropout(x)
...
class MsmcNet_RML2016_fuse2(nn.Module):
    '''
    Funcs:
        RML2016数据集下的MsmcNet网络结构定义，所有参数均按照MsmcNet的keras版本设置
    Network input size:
