...
parser.add_argument('--data_dir', help='the directory of the training data',default="db\projects\HRRP_Resnet50_test")
...
def read_mat(read_path):
    ...
    for i in range(0, len(folder_name)):
        ...
        class_data_normalization = trans_norm(all_mat_data)  # 归一化处理
...
def show_confusion_matrix(classes, confusion_matrix):
    ...
    plt.tight_layout()
    plt.savefig(args.work_dir+'/verification_confusion_matrix.jpg', dpi=1000)
...
def train_acc(epoch, acc):
    ...
    plt.tight_layout()
    plt.savefig(args.work_dir+'/training_accuracy.jpg', dpi=1000)
...
def val_acc(v_acc):
    ...
    plt.tight_layout()
    plt.savefig(args.work_dir+'/verification_accuracy.jpg', dpi=1000)
...
def inference_DNN(train_x, train_y, test_x, test_y, folder_name):
    ...
    save_hdf5_path = args.work_dir+'/model/'+model_naming+'.hdf5'
...
if __name__ == '__main__':
    ...
    if(args.net=='DNN'):
        inference_DNN(x_train, y_train, x_val, y_val, folder_name)
    elif(args.net=='CNN'):
        inference_CNN(x_train, y_train, x_val, y_val, folder_name)
    #cmd_generateStructureVis="python ../api/HRRP_vis_keras/hrrp_vis_keras.py ----save_model_info 1"
