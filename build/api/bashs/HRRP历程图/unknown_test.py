...
parser.add_argument('--data_dir', help='the directory of the project dir', default="db/datasets/local_dir/基于历程图数据的Resnet50网络")
...
def read_unkown_test_mat(unkown_test_mat_path, windows_length, windows_step):
    ...
    for i in range(0, len(file_name)):
        ...
        one_unkown_mat_data_win = HRRP_windows_cut(one_unkown_mat_data_norm, windows_length, windows_step)

        matrix_base = os.path.basename(one_unkown_mat_path)
        matrix_name = os.path.splitext(matrix_base)[0]  # 获取去除扩展名的.mat文件名称
        unkown_data[matrix_name] = one_unkown_mat_data_win
    return unkown_data
...
if __name__ == '__main__':
    ...
    HRRP_picture_length = args.windows_length  # HRRP历程图滑窗长度
    HRRP_picture_step = args.windows_step  # HRRP历程图滑窗步长

    unkown_data, folder_name = read_project(project_path, HRRP_picture_length, HRRP_picture_step)
