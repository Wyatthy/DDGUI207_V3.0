...
class AlexNet_change(nn.Module):
    configs = [1, 96, 256, 384, 384, 256]

    def __init__(self, width_mult=1):
        configs = list(map(lambda x: 3 if x == 3 else int(x * width_mult), AlexNet_change.configs))
        super(AlexNet_change, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv2d(configs[0], configs[1], kernel_size=(11, 1), stride=(2, 1)),
            nn.BatchNorm2d(configs[1]),
            nn.MaxPool2d(kernel_size=(3, 1), stride=2),
        ...
        self.feature_size = configs[5]
        self.fc = nn.Linear(configs[5], 512)
