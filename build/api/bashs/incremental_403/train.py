...
from model import AlexNet_change

from myNetwork import Network
from signalDataset import PretrainSignalDataset, IncrementSignalDataset, EvalDataset
from dataProcess import prepare_pretrain_data, prepare_incrementtrain_data, prepare_increment_data_reduce, mycopyfile
...
class Pretrain:
    def __init__(self, oldClassNumber, memorySize, preTrainEpoch, batch_size, learningRate, dataDimension, model_save_path, folder_names):
        super().__init__()
    ...
    def test_old(self, testLoader):
        self.model.eval()
        correct, total = 0, 0
        confusion_matrix = np.zeros((self.oldClassNumber, self.oldClassNumber))
    ...
    def train(self):
        ...
        if (self.dataDimension == 256):
            ...
            featureExtractor = AlexNet_change()
        ...
        for i in range(self.preTrainEpoch):
            ...
            sys.stdout.flush()
            test_accuracy, confusion_matrix, Y_pred, Y_val = self.test_old(test_dataloader)
    ...
    def save_memory(self, train_dataset, oldClassNumber, memorySize):
        ...
        print("oldClassNumber:",oldClassNumber)
...
class IncrementTrain:
    ...
    def train(self):
        prepare_incrementtrain_data(self.newClassNumber, self.allClassNumber-self.newClassNumber)
        ...
        res = torch.load(model_path + "Pretrain.pt")
        self.model.load_state_dict(res['model'])
        self.model.to(device)
        ...
        for task in classes:
            ...
            prepare_increment_data_reduce(task, self.reduce_sample, self.allClassNumber)
            ...
            train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=self.batchSize, num_workers=0)
            test_dataloader = DataLoader(test_dataset, shuffle=False, batch_size=self.batchSize, num_workers=0)
...
class Evaluation:
    ...
    def evaluate(self):
        ...
        old_dataloader = DataLoader(old_dataset, shuffle=False, batch_size=self.batch_size, num_workers=0)
        new_dataloader = DataLoader(new_dataset, shuffle=False, batch_size=self.batch_size, num_workers=0)
        ...
        if (self.data_dimension == 256):
            ...
            feature_extractor = AlexNet_change()
            # print("Data dimention is invaild!")
            # sys.stdout.flush()

        self.model = Network(self.all_class, feature_extractor)
        res = torch.load(model_path + "increment.pt")
